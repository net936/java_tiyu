...
public class OrderController {
    ...
    public APIResponse create(Order order) throws IOException {
        ...
        List<Order> existOrderList = service.getOrderList(order.getThingId(), order.getReceiverTime());
        if(null == existOrderList || existOrderList.size() > 0){
            return new APIResponse(ResponeCode.FAIL, "该场次已被其他用户预约");
        }
